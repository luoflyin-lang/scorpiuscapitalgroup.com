(function () {
  ...
  function renderProgress(value) {
    ...
    var display = state.progress >= 100
      ? '99'
      : (state.progress < 10 ? '0' : '') + (state.progress | 0);
  ...
  function start() {
    ...
    if (state.status === 'running' || state.status === 'finishing') return;
  ...
  function finish() {
    ...
    var finishDelay = Math.max(
      options.ghostTime,
      Math.max(options.minTime - elapsed, 0)
    );
  ...
  function finishInitialLoader() {
    if (!state.initialStarted || state.initialFinished) return;
